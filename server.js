...
const app = express();
const log = bunyan.createLogger({ name: 'Implicit Flow' });
...
//Set 1: Get the access token
app.get('/get/the/access/token', (req, res) => {

    const Authorization_Endpoint = `https://login.microsoftonline.com/${process.env.TENANT_ID}/oauth2/authorize`;
    const Response_Type = 'token';
    const Client_Id = process.env.CLIENT_ID;
    const Redirect_Uri = 'http://localhost:8000/give/me/the/access/token';
    const Scope = 'https://graph.microsoft.com/User.Read';
    const Resource = 'https://graph.microsoft.com/';
    //const Response_Mode = 'fragment';
    const State = 'ThisIsMyStateValue';

    // let url = `${Authorization_Endpoint}?response_type=${Response_Type}&response_mode=${Response_Mode}&client_id=${Client_Id}&redirect_uri=${Redirect_Uri}&scope=${Scope}&resource=${Resource}&state=${State}`;
    let url = `${Authorization_Endpoint}?response_type=${Response_Type}&client_id=${Client_Id}&redirect_uri=${Redirect_Uri}&scope=${Scope}&resource=${Resource}&state=${State}`;
...
app.get('/give/me/the/access/token', (req, res) => {
    ...
    res.render('access-token', { token: req.query.token, state: req.query.state });
});
